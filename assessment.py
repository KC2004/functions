...
def is_my_hometown(town_name):
    """ returns True, if town_name is same as mine. False otherwise.
        input case does not matter
    """ 
    my_hometown = "san jose"
...
def greet_person(hometown, first_name, last_name):
    """ if hometown is same as mine, greet person by name and let her know I am from same hometown
        else greet and ask what her home town is"
    """
    if is_my_hometown(hometown):
        print "%s, we're from the same place!" % full_name(first_name, last_name)

    else:
        print "%s, where are you from?"  % full_name(first_name, last_name)        
...
def calculate_price(base_price, state, tax = 0.05):
    ...
    if len(state) > 2:
        raise ValueError('state must have 2 alpha characters')

    if int(tax * 100) != tax * 100:
        raise ValueError('tax will have only 2 decimal places')
    
    sub_total_with_tax = base_price * (1 + tax)

    if state == 'CA':
        total = round((sub_total_with_tax * 1.03),1)
    elif state == 'PA':
        total = sub_total_with_tax + 2
    elif state == 'MA':
        if base_price < 100:
            total = sub_total_with_tax + 1
        else:
            total = sub_total_with_tax + 3
    else:
